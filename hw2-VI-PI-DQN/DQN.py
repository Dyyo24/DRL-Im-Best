...
class QNetwork():
    ...
    def __init__(self, state_dim, learning_rate):
        ...
        self.model.add(Dense(60, kernel_initializer='random_uniform', activation='relu', input_shape=(state_dim+1,)))
        self.model.add(Dense(50, kernel_initializer='random_uniform', activation='relu'))
        self.model.add(Dense(20, kernel_initializer='random_uniform', activation='relu'))
        # Output: Q-value        
...
class DQN_Agent():
    ...
    def epsilon_greedy_policy(self, q_values):
        ...
        prob = np.ones(len(q_values)) * self.eps/(len(q_values)-1)
        prob[action_opt] = 1 - self.eps
        action = np.random.choice(len(q_values), 1, p=prob).item()
        return action
    ...
    def train(self):
        ...
        for episode_num in range(self.episode_max):
            ...
            while not done:
                ...
                # calculate TD error based on r + gamma*Q*(s+1) - Q*(s)
                next_state_vec = np.tile(next_state, (self.env.action_space.n,1))
                next_state_q_vec = self.Qnet.predict(next_state_vec,all_action_vec)
            ...
            TD_error.append(TD_error_episode_average)
            np.save('TD.npy',TD_error)
            ...
            if episode_num % 100 ==0:
                ...
                print('TD error: ',TD_error_episode_average)
...
#    keras.backend.tensorflow_backend.set_session(sess)
    ...
    a = DQN_Agent(env_name, episode_max = 6000, epsilon= 0.5, gamma=1, C = 1000, learning_rate = 0.0001, memory_size=20000, burn_in = 10000)
    loss = a.train()
